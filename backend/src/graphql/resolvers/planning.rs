...
pub async fn start_planning(ctx: &Context<'_>, session_id: ID) -> Result<StudyPlan> {
    ...
    let plan_md = planning::generate_study_plan(
        pool,
        config,
        user_id,
        session_uuid,
    ...
    // Save the plan
    let plan = study_plans::create_study_plan(pool, session_uuid, &plan_md).await?;
...
pub async fn revise_study_plan(
    ...
    let revised_md = planning::revise_study_plan(
        pool,
        config,
        user_id,
        session_uuid,
    ...
    let new_plan = study_plans::create_plan_version(pool, session_uuid, &revised_md, &instruction).await?;
